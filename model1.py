...
class infoGAN(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, mnist_1, mnist_2, image_split, SUPERVISED=False): #log_dir
        self.sess = sess
        ...
        if dataset_name == 'mnist' or dataset_name == 'fashion-mnist':
            ...
            self.y_dim = 2 #4 #12         # dimension of code-vector (label+two features) #now just label
            self.c_dim = 1
            ...
            # code
            self.len_discrete_code = 2 #10  # categorical distribution (i.e. label)
    ...
    def train(self, config):
        ...
        self.test_codes = np.random.multinomial(1,[1-self.image_split, self.image_split], size=[self.batch_size])

        #self.test_codes = np.random.multinomial(1,self.len_discrete_code * [float(1.0 / self.len_discrete_code)], size=[self.batch_size])
        #self.test_codes = np.concatenate((self.test_codes, np.random.uniform(-1, 1, size=(self.batch_size, 2))), axis=1)

        #print(self.test_codes.shape)
            ...
            for idx in range(start_batch_id, self.num_batches):
                ...
                if self.SUPERVISED == True:
                    ...
                    batch_labels = np.random.multinomial(1,
                                                         [1-self.image_split, self.image_split],
                ...
                if epoch == config.epoch-1:
                    ...
                    directory = self.result_dir+'/Nums'+str(self.mnist_1)+str(self.mnist_2)+'/split'+str(self.image_split)+'/'

                    #directory = 'infogan_test/'
                    #directory = self.output_dir+'/Nums'+str(self.mnist_1)+str(self.mnist_2)+'/split'+str(self.image_split)+'/'
                    save_images_withcode(samples, samples.shape[0], 'train_{:02d}_{:04d}'.format(epoch, idx), directory, self.test_codes)
